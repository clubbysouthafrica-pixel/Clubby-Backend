  ...
  constructor(scope: Construct, id: string, props: MSC_PayFastNestedStackProps) {
    ...
    const save_card_details = new MSC_Lambda(this, `${id}-SaveCardDetails`, {
      ...
      envVariables: {
        ...
        RETURN_URL: `${process.env.ENVIRONMENT === "Dev" ? "http://localhost:5173/billing&usage?success=true" : `https://${process.env.DOMAIN}`}/billing&usage?success=true`,
        CANCEL_URL: `${process.env.ENVIRONMENT === "Dev" ? "http://localhost:5173/billing&usage?success=false" : `https://${process.env.DOMAIN}`}/billing&usage?success=false`,
