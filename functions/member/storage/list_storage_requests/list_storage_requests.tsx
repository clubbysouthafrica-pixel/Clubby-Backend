...
import { queryItems } from "./function_helpers/database_functions";
...
export const handler = async (event: any) => {
  ...
  try {
    ...
    const clubAccountId =
      ...
      query_string_params?.clubAccountId ??
      null;
    ...
    const items = await queryItems(tableName, "club_account_id = :clubId", {
    ...
    // Only return requests for the user on the specified club
    if (userId && clubAccountId) {
      filtered = filtered.filter(
        (it: any) =>
          (it.userId === userId || it.user_id === userId) &&
          (it.club_account_id === clubAccountId ||
            it.clubAccountId === clubAccountId),
      );
    } else if (userId) {
      filtered = filtered.filter(
        (it: any) => it.userId === userId || it.user_id === userId,
      );
    } else if (clubAccountId) {
      filtered = filtered.filter(
        (it: any) =>
          it.club_account_id === clubAccountId ||
          it.clubAccountId === clubAccountId,
